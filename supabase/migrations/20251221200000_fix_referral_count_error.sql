...
BEGIN
  ...
  -- 2. Lấy thông tin User (Rank, Referrer, Balance)
  -- FIX: Không select referral_count trực tiếp vì cột này không tồn tại
  SELECT 
    rank, 
    referred_by,
    balance
  ...
  -- FIX: Tính toán referral_count thủ công
  SELECT COUNT(*)::integer INTO v_user_referral_count
  FROM users
  ...
  -- 6. Logic xử lý kho Key hoặc Manual Delivery
