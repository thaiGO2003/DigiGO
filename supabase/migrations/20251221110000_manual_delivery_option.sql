-- 1. Thêm cột is_manual_delivery cho product_variants
ALTER TABLE product_variants ADD COLUMN IF NOT EXISTS is_manual_delivery BOOLEAN DEFAULT FALSE;

-- 2. Đảm bảo key_id trong transactions có thể NULL
ALTER TABLE transactions ALTER COLUMN key_id DROP NOT NULL;

-- 3. Cập nhật hàm purchase_product để hỗ trợ Manual Delivery
...
BEGIN
  ...
  IF v_variant_info.is_manual_delivery THEN
    -- MANUAL DELIVERY: Không check stock key
    ...
    FOR i IN 1..p_quantity LOOP
      ...
      -- Set key value hiển thị cho user là mã đơn hàng hoặc thông báo
      v_key_values[i] := 'Mã đơn (Chờ gửi key): ' || split_part(v_transaction_ids[i]::text, '-', 1);
    END LOOP;
