-- Migration to fix the incomplete purchase_product function and ensure total_sold is updated

-- 1. Restore the complete purchase_product function from the working version (20251221120000_manual_stock.sql)
-- This function includes manual stock support and all the original logic.
...
BEGIN
  ...
  -- 7. Update total_sold for the variant
  UPDATE product_variants
...
-- 2. Ensure the trigger for total_sold is in place (in case it was removed)
CREATE OR REPLACE FUNCTION update_total_sold_on_transaction()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.type = 'purchase' AND NEW.variant_id IS NOT NULL THEN
     -- Extract quantity from metadata if possible, or default to 1
     -- The metadata format is jsonb_build_object('quantity_in_order', ...)
     UPDATE product_variants 
     SET total_sold = total_sold + COALESCE((NEW.metadata->>'quantity_in_order')::int, 1)
...
DROP TRIGGER IF EXISTS trigger_update_total_sold ON transactions;
CREATE TRIGGER trigger_update_total_sold
AFTER INSERT ON transactions
FOR EACH ROW
EXECUTE FUNCTION update_total_sold_on_transaction();